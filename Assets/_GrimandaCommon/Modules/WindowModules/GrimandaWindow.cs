...
namespace Grimanda.Common
{
    ...
    public class GrimandaWindow : MonoBehaviour
    {
        ...
        public UnityEvent OnEnterAnimEnd;
            ...
            if (animateOnOpen)
            {
                ...
                for (int i = 0; i < animatableUIElements.Length; i++)
                {
                    switch (animatableUIElements[i].animType)
                    {
                        ...
                        case UIAnimType.MoveFromTopToMiddle:
                        case UIAnimType.MoveFromBottomToMiddle:
                            if (i + 1 == animatableUIElements.Length)
                            {
                                animatableUIElements[i].rectTransform.DOAnchorPos(animatableUIElements[i].anchoredPosInScreen, gameController.activeGameConfig.elementMovementTime, true).SetEase(gameController.activeGameConfig.ease).OnComplete(() =>
                                {
                                    OnEnterAnimEnd.Invoke();
                                }); 
                            }
                            else
                            {
                        ...
                        case UIAnimType.FadeAnim:
                            if (fadeActive)
                            {
                                if (i + 1 == animatableUIElements.Length)
                                {
                                    animatableUIElements[i].rectTransform.GetComponent<Image>().DOFade(animatableUIElements[i].endAlpha, gameController.activeGameConfig.elementMovementTime).OnComplete(() =>
                                    {
                                        OnEnterAnimEnd.Invoke();
                                    });
                                }
                        ...
                        case UIAnimType.ScaleUp:
                            if (i + 1 == animatableUIElements.Length)
                            {
                                 animatableUIElements[i].rectTransform.DOScale(animatableUIElements[i].endScale, gameController.activeGameConfig.elementMovementTime).OnComplete(() =>
                                 {
                                     OnEnterAnimEnd.Invoke();
                                 });
                            }
